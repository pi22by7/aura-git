...
// Body
async function _rpCreateOrder(amount, notes, currency = rpConfig.currency) {
	assert(typeof amount === "number");
	assert(amount >= 0);

	return rp.orders.create({ amount, currency, notes });
}

async function rpCreateOrderByScheme(scheme, notes = undefined) {
	if (!(scheme in rpConfig.schemes)) return null;

	return _rpCreateOrder(rpConfig.schemes[scheme].amount, notes);
}
